...
Usage:
    python scripts/ops/ci_detect_changes.py <before_sha> <after_sha>
    python scripts/ops/ci_detect_changes.py --json <before_sha> <after_sha>
...
def get_changed_versions(before: str, after: str):
    ...
    for stem in sorted(changed_stems, key=version_key):
        ...
        old_content = _git_show(before, f'ffmpeg/{stem}.yaml')
        new_content = _git_show(after, f'ffmpeg/{stem}.yaml')
        ...
        if is_family:
            ...
            if children:
                ...
                old = _get_revision(old_content)
                new = _get_revision(new_content)
                if old != new or is_new_file:
                    versions_to_build[stem] = new
        else:
            # Patch YAML (X.Y.Z) — build if revision changed or file is new
            old = _get_revision(old_content)
            new = _get_revision(new_content)
            if old != new or is_new_file:
                versions_to_build[stem] = new

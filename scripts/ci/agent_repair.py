...
# Reference to the running opencode process for signal handling
_opencode_proc: Optional[subprocess.Popen[str]] = None


# ── Signal handler (matches bash trap) ────────────────────────────────────────

def _signal_handler(signum: int, _frame: object) -> None:
    """Handle SIGTERM/SIGINT: warn, kill opencode, exit 1."""
    print("[WARNING] Workflow cancelled by user, terminating...", file=sys.stderr)
...
def main() -> None:
    ...
    # ── Install signal handlers (bash: trap '...' SIGTERM SIGINT) ─────────────

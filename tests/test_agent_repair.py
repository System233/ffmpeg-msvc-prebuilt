...
def _popen_mock(returncode=0):
    """Return a MagicMock that acts like subprocess.Popen."""
    proc = mock.MagicMock()
...
class TestAgentRepair(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        # Reset the global opencode process reference
        agent_repair._opencode_proc = None
    ...
    @mock.patch("agent_repair.subprocess.Popen")
    def test_4_successful_single_attempt(
        ...
        mock_popen.side_effect = _popen_mock(returncode=0)
    ...
    def test_6_max_retries_all_violations(
        ...
        """All 3 attempts have violations; post-loop also has violations → exit 1."""
        mock_run.side_effect = _make_run_side_effect({
